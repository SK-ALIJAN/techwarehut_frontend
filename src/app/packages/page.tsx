      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 -mt-6 relative z-10">
        ...
        <div className="mt-10">
          {isLoading ? (
            ...
            <div className="bg-red-50 border border-red-100 rounded-2xl p-8 text-center text-red-650 max-w-md mx-auto flex flex-col items-center gap-3">
              <AlertCircle className="w-10 h-10 text-red-500" />
              <h3 className="font-bold text-lg">Error Loading Packages</h3>
              <p className="text-sm text-red-505/80">Please check your network connection or try again later.</p>
            </div>
            ...
            <div className="bg-white border border-slate-100 rounded-2xl p-16 text-center text-slate-500 max-w-md mx-auto flex flex-col items-center gap-4 shadow-sm">
              ...
              <h3 className="font-bold text-lg text-slate-805">No Packages Found</h3>
              <p className="text-sm text-slate-500">We couldn't find any packages matching "{search}". Try searching for another destination.</p>
              <button 
                onClick={() => setSearch('')}
                className="mt-2 bg-primary-600 hover:bg-primary-505 text-white font-semibold px-4 py-2 rounded-xl text-sm transition"
              >
                Clear Search
              </button>
            </div>
            ...
            <>
                      ...
                      <div className="bg-white rounded-2xl border border-slate-100 overflow-hidden shadow-sm hover:shadow-lg transition-all duration-300 flex flex-col h-full transform hover:-translate-y-1">
                        ...
                        <div className="relative h-48 w-full overflow-hidden bg-slate-200 shrink-0">
                          ...
                          <div className="absolute bottom-3 right-3 bg-white/90 backdrop-blur-md px-2.5 py-1 rounded-lg text-xs font-semibold text-slate-800 shadow-sm flex items-center gap-1">
                            <Clock className="w-3.5 h-3.5 text-primary-505" />
                        ...
                        <div className="p-5 flex flex-col flex-grow">
                          ...
                          <div className="pt-4 border-t border-slate-50 flex items-center justify-between mt-auto">
                            <div>
                              <span className="text-[10px] text-slate-400 block uppercase tracking-wider font-semibold">Price per person</span>
              ...
              {pagination && pagination.totalPages > 1 && (
                <div className="mt-12 flex flex-wrap justify-center items-center gap-3 sm:gap-4 text-xs sm:text-sm">
                  <button
                    disabled={page === 1}
                    onClick={() => setPage((p) => Math.max(1, p - 1))}
                    className="px-3 sm:px-4 py-2 border border-slate-200 bg-white hover:bg-slate-50 rounded-xl font-semibold text-slate-700 disabled:opacity-50 disabled:cursor-not-allowed transition flex items-center gap-1 shadow-2xs"
                  >
                    &larr; Prev
                  </button>
